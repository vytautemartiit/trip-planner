...
function App() {
  ...
  const BASE_URL = 'https://supreme-computing-machine-r777rj7qj9vq29w6-8000.app.github.dev'
  ...
  <div className="input-row">
    <input
      ...
      placeholder="2"
      value={countries}
      onChange={(e) => setCountries(Number(e.target.value) || 1)}
    />
  </div>
</div>
        <div className="field">
          ...
          : results.map((trip, index) => (
              <div key={index} className="ticket">
                <div className="ticket-top">
                  ...
                  <span className="ticket-no">№MULTI-{trip.countries.length}</span>
                </div>

                <div className="destination">{trip.countries.join(' · ')}</div>
                <div className="days-label">
                  {trip.total_days} d. bendrai ({trip.breakdown.map((b) => `${getCode(b.destination)}: ${b.days} d.`).join(', ')})
                </div>
